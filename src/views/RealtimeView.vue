    ...
    <div class="max-w-5xl mx-auto space-y-6">
      ...
      <div class="flex justify-between items-end border-b border-gray-200 pb-4">
        <div class="flex items-center gap-4">
          <h1 class="text-2xl font-bold text-red-800">MFU Vision</h1>
          <span class="text-gray-500 font-medium">Live Feed</span>
        </div>
      ...
      <div class="space-y-8">
        ...
        <div v-for="cam in filteredCameras" :key="cam.id" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden transition-all duration-300">
          <div class="relative aspect-[16/9] bg-gray-800 flex items-center justify-center overflow-hidden">
            ...
            <img :src="cam.image" class="absolute inset-0 object-cover w-full h-full opacity-80" alt="Video Feed">
            
            <div class="absolute top-4 left-4 z-10">
              <div class="bg-black/70 text-white text-[10px] font-mono px-3 py-1.5 rounded flex items-center gap-2 border border-white/10">
            ...
            <div class="absolute top-4 right-4 z-20 flex items-center gap-2">
              <div class="bg-black/70 text-white text-[10px] font-mono px-2 py-1 rounded border border-white/10 hidden sm:block">FPS <span class="font-bold">{{ cam.fps }}</span></div>
              <div class="bg-black/70 text-white text-[10px] font-mono px-2 py-1 rounded border border-white/10 hidden sm:block">LATENCY <span class="font-bold">{{ cam.latency }}</span></div>
            ...
            <div class="absolute border-2 border-red-600 bg-red-600/10 z-20 transition-all duration-300" style="top: 20%; left: 45%; width: 18%; height: 25%;">
              <div class="absolute bottom-0 left-0 bg-red-600 text-white text-[8px] font-bold px-1 py-0.5 whitespace-nowrap">NO HELMET: {{ cam.helmetScore }}</div>
            </div>

            <div class="absolute border-2 border-yellow-400 bg-yellow-400/10 z-20" style="bottom: 2%; right: 18%; width: 12%; height: 10%;">
              <div class="absolute top-0 left-0 bg-yellow-400 text-black text-[8px] font-bold px-1 py-0.5 whitespace-nowrap">PLATE: {{ cam.plate }}</div>
            </div>
      ...
      <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl">
        ...
        <div class="space-y-4">
          <div>
            ...
            <input v-model="newCamName" type="text" placeholder="e.g. DORM_FRONT_03" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-800 focus:border-red-800 outline-none">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">RTSP Stream URL (Optional)</label>
            <input type="text" placeholder="rtsp://admin:pass@192.168.1.100/stream" class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 focus:ring-2 focus:ring-red-800 focus:border-red-800 outline-none">
          </div>
        </div>

        <div class="mt-8 flex justify-end gap-3">
          <button @click="closeModal" class="px-5 py-2 text-sm font-medium text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">Cancel</button>
          <button @click="addNewCamera" class="px-5 py-2 text-sm font-medium text-white bg-red-800 hover:bg-red-900 rounded-lg transition-colors shadow-sm">
            Connect Camera
          </button>
        </div>
      </div>
